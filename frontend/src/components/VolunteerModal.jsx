...
export default function VolunteerModal({ postId, onComplete, onClose }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_URL}/users/search/${searchQuery}`,
        { credentials: 'include' }
      )
  ...
  const handleSubmit = () => {
    if (
      !window.confirm(
        'Are you sure you want to mark this post as completed, with the selected user as a volunteer?'
      )
    ) {
      return
    }
  ...
  return (
    <div className='modal-overlay'>
      <div
        className='volunteer-search-modal'
